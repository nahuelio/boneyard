/**
*	com.boneyard.ioc.plugins.HTMLPlugin Class Tests
...
define(['ioc/plugins/html',
	...
	describe('com.boneyard.ioc.plugins.HTMLPlugin', function() {
		...
		describe('#getPackageFullPath()', function() {
			...
			it('Should return package full path to the file', function() {
				...
				expect(result).to.be('text!/base/test/com/boneyard/ioc/html/cart.json');
			});
		...
		describe('#parsePackage()', function() {
			...
			it('Should return package suitable for factory enqueueing', function() {
				...
				expect(result.path).to.be('text!/base/test/com/boneyard/ioc/html/cart.json');
		...
		describe('#load()', function() {
			...
			it('Should load a list of valid packages', function(done) {
				var getPackageFullPathStub = sinon.stub(this.plugin, 'getPackageFullPath')
					.returns('text!/base/test/com/boneyard/ioc/html/checkout.json');
				...
				var result = this.plugin.load(['checkout'], _.bind(function(packages) {
					...
					expect(Boneyard.__html__.checkout.payment).to.be.ok();
					expect(Boneyard.__html__.checkout.payment.creditcard).to.be.a('string');
					done();
				}, this));
			...
			it('Should load a list of valid packages (with no callback)', function(done) {
				...
				var getPackageFullPathStub = sinon.stub(this.plugin, 'getPackageFullPath')
					.returns('text!/base/test/com/boneyard/ioc/html/product.json');
		...
		describe('#run()', function() {
			...
			it('Should executes plugin logic', function() {
				...
				expect(Boneyard.load).to.be.a('function');
				expect(Boneyard.html).to.be.a('function');
