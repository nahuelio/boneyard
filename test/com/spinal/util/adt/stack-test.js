/**
*	com.spinal.util.adt.Stack Class Tests
*	@author Patricio Ferreira <3dimentionar@gmail.com>
**/
define(['core/spinal', 'util/adt/stack'], function(Spinal, Stack) {

	describe('com.spinal.util.adt.Stack', function() {
		...
		**/
		describe('#new()', function() {
			...
			it('Should return an com.spinal.util.adt.Stack Instance', function() {
				this.testSimple = new Stack();
				expect(this.testSimple).to.be.ok();
				expect(this.testSimple.size()).to.be.equal(0);
			});

			it('Should return an com.spinal.util.adt.Stack (initial elements)', function() {
				this.testSimple = new Stack([{ name: 'foo' }, { name: 'bar' }]);
				expect(this.testSimple).to.be.ok();
				expect(this.testSimple.size()).to.be.equal(2);
			});

			it('Should return an com.spinal.util.adt.Stack (With interface)', function() {
				this.testInterface = new Stack([], { interface: Spinal.SpinalClass });
